...
<script>
    $(function () {
        $.ajax({
            type:"get",
            url:"${pageContext.request.contextPath}/getLevel?level=${category.level}",
            async:true,
            success:function(data){
                var  data=JSON.parse(data);
                $.each(data,function (index,item) {
                    var oP=$("<option class="+item.name+">"+item.name+"</option>");
                    $(oP).val(item.id);
                    $('#parent_ida').append(oP);
                })
                $("option[value='${category.level}']").prop("selected",true);
                $("option[class='${category.belong}']").prop("selected",true);
            }
        })
...
<body>
<div class="container">
    <form class="form-horizontal" id="myform" action="${pageContext.request.contextPath}/admin/category/update" method="post">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-3">
                ...
                <input type="text" class="form-control" id="address" name="name" value="${category.name}">
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-3">
                <input type="text" class="form-control" id="address" name="cDesc" value="${category.cDesc}">
            </div>
        </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-3">
                <select class="form-control" id="parent_ida" name="parentId">
                </select>
            </div>
        </div>
